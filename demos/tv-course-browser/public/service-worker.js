...
self.addEventListener('fetch', function (event) {
	...
	// _debug('Processing request for', request.url)

	if (!DOMAIN_WHITELIST.includes(url.hostname)) {
		// _debug('Request URL does not match domain whitelist:', url.hostname)
		return
	}

	const exts = EXTENSION_WHITELIST.join('|')
	if (!request.url.match(new RegExp(`\\.(${exts})$`, 'gi'))) {
		// _debug('Request URL does not match extension whitelist:', request.url)
		return
	}

	// _debug('Caching request for', request.url)

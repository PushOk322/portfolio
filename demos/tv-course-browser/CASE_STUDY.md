...
**Tech:** React 18 · webpack 5 · Zustand · Tizen · webOS · norigin-spatial-navigation · SCSS

---

## What it is

A television app for a video-course platform: browse a hero carousel and horizontal
rows, open a course, play a session. One React codebase, packaged for Samsung Tizen and
LG webOS. **Use the arrow keys** — there is nothing to click, and that is the point.

115 of 214 commits are mine. I also took the app's Lighthouse score from 62 to 95 and
removed the UI latency that TV hardware exposes much faster than a laptop does.
...
— without it, focus walks off-screen and the app looks frozen. What differs between the
two TV platforms — the back key, history, exiting the app — sits behind two small
adapters, `useTizenHistory` and `useWebOsHistory`, so the rest of the app never asks
which television it is running on.
